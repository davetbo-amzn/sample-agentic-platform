...
echo "Packaging Lambda function for AgentCore Runtime deployment using Docker..."

# Clean and recreate the build directory to ensure no previous artifacts remain
[ -d build ] && rm -rf build 
[ -f agentcore-runtime-lambda.zip ] && rm -f agentcore-runtime-lambda.zip
...
docker run --rm \
  ...
  -c "
    ...
    pip install -t /asset-output -r /asset-input/agentic_platform/service/agentcore/runtime/lambda-requirements.txt && \
    ...
    zip -r agentcore_runtime_client.zip . && \
    echo 'Lambda package created successfully in /asset-output/agentcore_runtime_client.zip'
  "

# Copy the zip file to the project root for Terraform to use
cp build/agentcore_runtime_client.zip agentcore-runtime-lambda.zip

echo "Package created successfully: agentcore-runtime-lambda.zip"
